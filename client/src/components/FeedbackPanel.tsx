...
export default function FeedbackPanel({ feedback, executionResult }: FeedbackPanelProps) {
  if (!executionResult) {
    return null;
  }
  ...
  const statusColor = statusColors[executionResult.status as keyof typeof statusColors] || 'text-gray-600';
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-6 mt-6">
      ...
      <div className="mb-4">
        <p className="font-medium">Status: <span className={statusColor}>{executionResult.status}</span></p>
        {executionResult.executionTime !== undefined && (
          <p className="text-gray-600">Execution Time: {executionResult.executionTime} ms</p>
        )}
        {executionResult.memory !== undefined && (
          <p className="text-gray-600">Memory Used: {executionResult.memory} KB</p>
        )}
      </div>
